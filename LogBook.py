...
class BinaryTree:
    ...
    # Added display method inside BinaryTree class
    def display(self, root=None, is_top=True):
        if is_top:
        ...
        self.display(root.left, is_top=False)
        print("Time: {} | Visitor: {}".format(root.entry_time, root.visitor_name))
        self.display(root.right, is_top=False)
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            entry_time = raw_input("Enter entry time (e.g., 09:30 AM): ").strip()
            visitor_name = raw_input("Enter visitor name: ").strip()
            tree.insert(entry_time, visitor_name)
            print("Log entry added successfully!")
            tree.display()

        elif choice == '2':
            entry_time = raw_input("Enter entry time to delete (e.g., 09:30 AM): ").strip()
            ...
            entry_time = raw_input("Enter entry time to search (e.g., 09:30 AM): ").strip()
            result = tree.search(tree.root, entry_time)
            if result:
                print("Found! Time: {} | Visitor: {}".format(result.entry_time, result.visitor_name))
            else:
                print("Log entry not found.")
            tree.display()
            ...
            print("2. Preorder")
            print("3. Postorder")
            t_choice = raw_input("Enter traversal choice (1-3): ").strip()
            ...
            print("Total entries in log book: {}".format(total))
            tree.display()
